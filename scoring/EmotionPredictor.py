...
def test_classifier(df):
    ...
    happy = get_prediction(classifier_path, data, 0.4862)  # found during training
    conf_mat(happy,labels,'salarian_test')
    score_heatmap(happy,labels, 'salarian_test')
...
def make_random_forest(feats, labels, groups, scoring):
    ...
    random_forest = RandomizedSearchCV(RandomForestClassifier(), param_grid, n_iter=15, scoring=scoring,
                                       n_jobs=multiprocessing.cpu_count(), cv=groupkfold)
...
def make_emotion_data(emotion, df):
    ...
    groups = data['session']  # this is for grouping of train-test set later on
...
def load_patient(patient_dir):
    try:
        # curr_df = pd.read_hdf(os.path.join(patient_dir, 'hdfs', 'au_w_anno.hdf'))
...
if __name__ == '__main__':
    ...
    if args.refresh:
        ...
        # df = pd.DatFrame(res, interleave_partitions=True)
    ...
    if args.test:  # this currently only works for a single emotion, namely happy
        test_classifier(df)
    else:
