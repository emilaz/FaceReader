...
def crop_and_resize(vid, width, height, x_min, y_min, directory,
    ...
    subprocess.Popen(
        ...
        'ionice -c2 -n4 ffmpeg -y -loglevel quiet -i {0} -filter:v \"crop={1}:{2}:{3}:{4}\" -c:a copy -crf 17 {5}'
        .format(vid, str(width), str(height), str(x_min), str(y_min),
                crop_vid),
        shell=True).wait()
    if not to_img:
        subprocess.Popen(
            'ionice -c2 -n4 ffmpeg -y -loglevel quiet -i {0} -vf scale={2}*iw:{2}*ih  -c:a copy {1}'.format(
        ...
        for i in range(self.num_frames):
            ...
            if len(read_arr) > i:
                ...
                if confidence > .25:
                    ...
                    norm_coords = normalize_to_camera(
                    ...
                    x_coords = np.clip(
                        np.array([x_min, x_max]), 0, self.vid_width)
                    y_coords = np.clip(
                        np.array([y_min, y_max]), 0, self.vid_height)
                    x_min = x_coords[0]
                    x_max = x_coords[1]
                    y_min = y_coords[0]
                    y_max = y_coords[1]
    ...
    def make_read_arr(self, f, num_constraint=None):
        ...
        for index, num in enumerate(read_arr):
            for val_index, val in enumerate(num):
                read_arr[index][val_index] = read_arr[index][
                    val_index].replace('(', '')
                read_arr[index][val_index] = read_arr[index][
                    val_index].replace(')', '')
...
def normalize_to_camera(coords, crop_coord, scaled_width, scaled_height):
    if sum(crop_coord) <= 0:
        # Original size was 256
        rescale_factor = (scaled_width / 256, scaled_height / 256)
    else:
        rescale_factor = ((crop_coord[1] - crop_coord[0]) / 256.0,
                          (crop_coord[3] - crop_coord[2]) / 256.0)
    norm_coords = [
