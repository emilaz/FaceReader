"""
.. module:: HappyVidMarker
    :synopsis: A script for marking all videos with happy graph
"""
...
def bar_movie(vid, vid_dir, times, corr, replace_input=True):
    ...
    :param vid: Video
    :type vid: str
    :param vid_dir: Video directory
    :type vid_dir: str
    :param times: Array with times for vid
    :param corr: Array with corresponding "happy" probability for each time
    :param replace_input: Whether to replace the input video or output new video
    """
    ...
    subprocess.Popen(
        'ffmpeg -loglevel quiet -y -i {0} -vf "movie={1}, '
...
def mark_vid_dir(out_q, vid_dir):
    """
    Use emotion data to put the happy bar movie in the video directory
    :param out_q: for multiprocessing, queue to put the vid_dir in to indicate done
    :param vid_dir: Video directory to mark
    :effects: Puts {vid_dir: sum(emotion probs)/len(emotion probs)} into the out_q
    """

    if 'inter_out.avi' not in os.listdir(vid_dir):
        shutil.copy(
            get_vid_from_dir(vid_dir), os.path.join(vid_dir, 'inter_out.avi'))
    ...
    subprocess.Popen(
        'ffmpeg -loglevel quiet -y -i {1} -i {2} -filter_complex "[0:v]pad=iw*2:ih[int];[int]['
        '1:v]overlay=W/2:0[vid]" -map [vid] -c:v libx264 -crf 23 -preset veryfast {0}'
        .format(
            os.path.join(vid_dir, 'combined_out.avi'), vid,
            os.path.join(vid_dir, 'out.avi')),
        shell=True).wait()
